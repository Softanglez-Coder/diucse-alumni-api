...
export class CommitteeDesignationService extends BaseService<CommitteeDesignationDocument> {
  ...
  async createDesignation(
    createDto: CreateCommitteeDesignationDto,
  ...
  async updateDesignation(
    id: string,
    updateDto: UpdateCommitteeDesignationDto,
  ): Promise<CommitteeDesignationDocument> {
  ...
  async getDesignationsByCommittee(
    committeeId: string,
  ): Promise<CommitteeDesignationDocument[]> {
    return await this.committeeDesignationRepository.findByCommitteeId(
      committeeId,
    );
  }

  async assignMember(
    assignDto: AssignCommitteeMemberDto,
  ): Promise<CommitteeMemberDocument> {
    const { committeeId, designationId, userId, assignedDate, notes } =
      assignDto;

    // Check if user already has an active assignment in this committee
    const existingAssignment =
      await this.committeeMemberRepository.findActiveByUserAndCommittee(
        userId,
        committeeId,
      );
    if (existingAssignment) {
      throw new BadRequestException(
        'User already has an active assignment in this committee',
      );
    }

    // Check if designation already has an active member in this committee
    const existingDesignationMember =
      await this.committeeMemberRepository.findActiveByDesignationAndCommittee(
        designationId,
        committeeId,
      );
    if (existingDesignationMember) {
      throw new BadRequestException(
        'This designation already has an active member assigned',
      );
    }
  ...
  async unassignMember(
    memberId: string,
    unassignDto: UnassignCommitteeMemberDto,
  ): Promise<CommitteeMemberDocument> {
    ...
    const updateData = {
      isActive: false,
      unassignedDate: unassignDto.unassignedDate
        ? new Date(unassignDto.unassignedDate)
        : new Date(),
  ...
  async getCommitteeMembers(
    ...
    return await this.committeeMemberRepository.findByCommitteeId(
      committeeId,
      !includeInactive,
    );
  }
  ...
  async getUserCommitteeHistory(
    ...
    return await this.committeeMemberRepository.findByUserId(
      userId,
      !includeInactive,
    );
  }
  ...
  async getUserActiveRoles(userId: string): Promise<string[]> {
    const activeAssignments = await this.committeeMemberRepository.findByUserId(
      userId,
      true,
    );
    ...
    for (const assignment of activeAssignments) {
      if (
        ...
        typeof assignment.designationId === 'object' &&
        'roles' in assignment.designationId
      ) {
